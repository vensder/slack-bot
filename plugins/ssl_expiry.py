...
def get_ssl_expiry_datetime(hostname):
    ...
    # 2016-04-19 23:59:59 # <class 'datetime.datetime'>
    ...
    if trigger_phrase in data['text']:
        ...
        if mask.search(text):
            ...
            try:
                ...
                if delta.days < 30:
                    emoji = ':warning:'
                output_string = 'Certificate for domain ' + domain + \
                    ' will expire in ' + \
                    str(delta.days) + ' days (' + str(expiry_datetime) + ')'
                print(output_string)
                ...
                emoji = ':exclamation:'
                output_string = 'SSL checker' + str(e) + ': ' + domain
                web_client.chat_postMessage(
                    icon_emoji=emoji,
                    channel=channel_id,
                    text=output_string,
